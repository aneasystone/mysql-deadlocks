...
这里有一个有意思的点，如果 account_id = 561 不存在且大于 account_id 列的最大记录，那么在 supremum 这条记录上加 lock_mode X 锁（Next-key锁）；如果 account_id = 561 不存在且是个中间值，则加的是 gap 锁（locks gap before rec）。
